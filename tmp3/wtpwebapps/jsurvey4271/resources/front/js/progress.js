...
$.ajax({
	 type:"post",
	 async:false, 
	 url:contextpath+"/front/dczj/findRightSocre.do",
	 data:"dczjid="+projsurveyid,
	 success:function(result){
	 	if(result.success){
...
function validateQues(){
	...
	if(mustfillquesid >0){
		if(mustfilltype == 0 || mustfilltype == 6){
		...
		}else if(mustfilltype == 1 || mustfilltype == 7){
			 var checkboxnum = 0 ;
...
function progress(){
	...
	for ( var i in jsonquesid) {
		if (jsonquesid[i].type == 0 || jsonquesid[i].type == 6) {
			var radioname = "surveyAnsw"+ jsonquesid[i].quesid;
			var radioval = $('input:radio[name="'+radioname+'"]:checked').val();   
			if(radioval != null){
				count++;
			}
		}else if(jsonquesid[i].type == 1 || jsonquesid[i].type == 7){
